...
matrix = np.empty((len(X_train_datasets_5d),7))
for j in range(len(X_train_datasets_5d)):

    distance = []


    linSep, decBoundComp, N_inter,hyperCentres,idx1,idx2 = extractDecisionBoundaryMeasures(X_train_datasets_5d[j], y_train_datasets_5d[j], seed=0)
    ...
    samplesPerGroup, groupsPerClass, sdVar = extractTopologyMeasures(X_train_datasets_5d[j], y_train_datasets_5d[j])
    matrix[j][0] = round(linSep,3)
    matrix[j][1] = round(decBoundComp,3)
    matrix[j][2] = round(hyperCentres.shape[0],3)
    matrix[j][3] = round(samplesPerGroup,3)
    matrix[j][4] = round(N_inter,3)
    matrix[j][5] = round(np.mean(distance),3)
    matrix[j][6] = round(np.std(distance),3)
    
# np.savetxt('E:\Internships_19\Internship(Summer_19)\Imbalanced_class_classification\Class_Imabalanced_Learning_Code\CIL Code\RESULTS\Good_data_metrics_5d_part3.csv', matrix.tolist() ,delimiter=',',fmt='%f')
